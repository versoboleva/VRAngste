...
//# define CLOUDSTOYFREE 

using UnityEngine;
using UnityEditor;

namespace JocyfCloudsToy
{
	...
	public class CloudsToyEditor : Editor
	{
		...
		private SerializedProperty maxWidthCloud;
		private SerializedProperty maxHeightCloud;
		...
		private SerializedProperty isFadeInOut;
		private SerializedProperty fadeRatio;
		private SerializedProperty alphaRange;
		...
		// Rate me PlayerPrefs Values
		public enum RateOptions { Rated = 0, Never = 1, Later = 2 }
		public RateOptions rateOptions = RateOptions.Later;     // Rate option picked by the user.
		public int dayNumber = 1;   // Day number when use clicked onver any rate option.

		public Texture2D myCloudsToyIcon;
		...
		public void OnEnable()
		{
			...
			// First Time we use CloudsToy, don't show the rate button right now (wait untill tomorrow)
			...
			{
				//myCloudsToyIcon = Resources.Load<Texture2D>("CloudsToyIconFreev2"); // NOP.
		...
		// Reset the Rate keys (just to check everything is allright.
		private void ResetKeys()
		{
			...
			GUIContent contentRateMe = new GUIContent("Reset", "Reset!!!");
		...
		private void DrawRateMe()
		{
			...
			if (GUI.Button(buttonRect, contentRateMe))
			{
				if (CloudsToy.isCloudsToyFree)
				{
					Application.OpenURL("https://www.assetstore.unity3d.com/en/#!/content/35559");  // CloudsToy Free
				}
				else
				{
		...
		private void DrawCloudsToyIcon()
		{
			...
			GUIStyle newGUIStyle = new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter };
		...
		public override void OnInspectorGUI()
		{
			...
			//==========================================================
			//
			// ResetKeys(); // Used just for testing the rate function.
			//
			//==========================================================

			// Starts Drawing the Rate CloudsToy Button. Only if Rate option is later and the day number is now today's number.
			// So rate buttons only show once every day at maximum.
			//Debug.Log("rateOptions: " + rateOptions + " dayNumber: "+ dayNumber);

			switch (rateOptions)
			{
				case RateOptions.Rated:
				case RateOptions.Never:
					break;
				case RateOptions.Later:
					if (dayNumber != System.DateTime.Today.Day)
						DrawRateMe();
					break;
			}
			...
			switch (toolbarOptions)
			{
				...
				case 0:
					...
					if (GUI.changed)
					{
						switch (cloudPreset.enumValueIndex)
						{
							...
							case (int)CloudsToy.TypePreset.Fantasy:
								...
								break;
							case (int)CloudsToy.TypePreset.Dark:
								CloudSystem.SetPresetDark();
								break;
							case (int)CloudsToy.TypePreset.Cotton:
								CloudSystem.SetPresetCotton();
								break;
							case (int)CloudsToy.TypePreset.CottonLight:
								CloudSystem.SetPresetCottonLight();
								break;
							case (int)CloudsToy.TypePreset.OrangeOil:
								CloudSystem.SetPresetOrangeOil();
								break;
							case (int)CloudsToy.TypePreset.None:
							default:
								CloudSystem.SetPresetNone();
								break;
						}
						EditorUtility.SetDirty(target);
						//GUI.changed = false;
					...
					if (showAdvancedSettings)
					{
						...
						sizeFactorPart.floatValue = EditorGUILayout.Slider(contentCloud, sizeFactorPart.floatValue, 0.1f, 10.0f);
						...
						emissionMult.floatValue = EditorGUILayout.Slider(contentCloud, emissionMult.floatValue, 0.1f, 10.0f);
						contentCloud = new GUIContent("  Show Debug: ", "Show the Debog.Log messages in console.");
						EditorGUILayout.PropertyField(showDebug, contentCloud);
						EditorGUILayout.Separator();
					}
					GUI.color = myColor;
					EditorGUILayout.Separator();
					...
					maxWidthCloud.intValue = EditorGUILayout.IntSlider(contentCloud, maxWidthCloud.intValue, 10, 1000);
					contentCloud = new GUIContent("  Height: ", "Maximum height of each cloud");
					maxHeightCloud.intValue = EditorGUILayout.IntSlider(contentCloud, maxHeightCloud.intValue, 5, 500);
					...
					EditorGUILayout.Separator();
					contentCloud = new GUIContent("  Fade In/Out Cloud", "Each cloud can be animated making it fade In / Out.");
					EditorGUILayout.PropertyField(isFadeInOut, contentCloud);
					if (isFadeInOut.boolValue)
					{
						contentCloud = new GUIContent("  Fade Ratio: ", "The duration of the Fade In / Out effect cycle - it's a ping-pong animation type.");
						fadeRatio.floatValue = EditorGUILayout.Slider(contentCloud, fadeRatio.floatValue, 1.0f, 120.0f);
						contentCloud = new GUIContent("  Fade Range: ", "Alpha variation range to make the Fade In / Out effect.");
						alphaRange.floatValue = EditorGUILayout.Slider(contentCloud, alphaRange.floatValue, 0.0f, 1.0f);
					}
				...
				case 1:
					...
					if (ProcText)
					{
						...
						GUILayout.Box(ProcText.MyTexture, GUILayout.Width(MyWidth), GUILayout.Height(MyWidth));
						EditorGUILayout.Separator();
						EditorGUILayout.Space();
						if (CloudSystem.PT1UseAlphaTexture)
						{
							...
							GUILayout.Box(ProcText.MyAlphaDrawTexture, GUILayout.Width(MyWidth), GUILayout.Height(MyWidth));
						}
						EditorGUILayout.Separator();
						EditorGUILayout.EndHorizontal();
						EditorGUILayout.Separator();
					...
					if (GUI.Button(buttonRectPT1, contentCloud))
					{
						...
						if (ProcText) { CloudSystem.PT1CopyParameters(); }
					}
					EditorGUILayout.EndHorizontal();
					EditorGUILayout.Separator();
					EditorGUILayout.Separator();
